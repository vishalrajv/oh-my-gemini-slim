...
**First, check if `.gemini/codemap.json` exists in the repo root.**

If it does not exist, check for legacy state at `.gemini/cartography.json`.

If legacy state exists: move `.gemini/cartography.json` to `.gemini/codemap.json`, then continue with change detection.

If `.gemini/codemap.json` exists: Skip to Step 3 (Detect Changes) - no need to re-initialize.
...
This creates:
- `.gemini/codemap.json` - File and folder hashes for change detection
...
**Gemini CLI auto-loads `AGENTS.md` into agent context on every session.** To ensure agents automatically discover and use the codemap, update (or create) `AGENTS.md` at the repo root:
...
5. Returns agent configs to Gemini CLI
...
A high-performance, low-latency agent orchestration plugin for Gemini CLI, focusing on specialized sub-agent delegation and multiplexer-assisted child sessions.

## System Entry Points
- `src/index.ts`: Plugin initialization and Gemini CLI integration.
