...
- Generate and evolve change-aware codemap state artifacts (`.gemini/codemap.json`) and scaffold placeholders (`codemap.md`).
...
- Persistence model: JSON state at `.gemini/codemap.json` with `metadata`, `file_hashes`, and `folder_hashes`.
...
- `cmdInit()` computes include/exclude candidate sets using `selectFiles()` and writes:
  1) `.gemini/codemap.json` via `saveState()`
...
- `codemap` skill invocation path in SKILL workflow is explicit: Step 1 checks `.gemini/codemap.json` or `.gemini/cartography.json`, then Step 2/3 selects init or incremental path.

## Integration

- Installed under Gemini CLI through `src/cli/custom-skills.ts` as `name: 'codemap'`, `sourcePath: 'src/skills/codemap'`.
- `src/cli/install.ts` copies this directory into the user skill directory; Gemini CLI executes `scripts/codemap.mjs` from that context.
